...
export default function CareSeekersPage() {
  ...
  const { data, isLoading } = useQuery<ApiResponse>({
    queryKey: ['care-seekers', pagination],
    queryFn: async () => {
      const res = await api.get('/users/careseekers', {
        params: {
  ...
  const suspendMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['care-seekers'] }),
  })

  const columns: ColumnDef<CareSeeker>[] = [
    {
      header: 'Name',
      cell: ({ row }) => {
      ...
      cell: ({ row }) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={(e) => { e.stopPropagation(); router.push(`/care-seekers/${row.original.id}`) }}>
              View Details
            </DropdownMenuItem>
            <DropdownMenuItem
              onClick={(e) => {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Care Seekers" description="Manage all care seeker accounts" />

      <DataTable
        columns={columns}
        data={data?.careSeekers ?? []}
        total={data?.pagination?.total ?? 0}
        pagination={pagination}
        onPaginationChange={setPagination}
        isLoading={isLoading}
        onRowClick={(row) => router.push(`/care-seekers/${row.id}`)}
      />
    </div>
  )
}
